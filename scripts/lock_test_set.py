...
README_FILE = TEST_ROOT / "README.md"
...
def main() -> None:
        ...
        manifest_rows.append(
            {
                ...
                "test_file": str(destination.relative_to(TEST_ROOT)),
    ...
    summary = "\n".join(f"- `{stratum}`: {count}" for stratum, count in counts.items())
    README_FILE.write_text(
        "# Frozen Week-1 OCR test set\n\n"
        "This directory contains the fixed test set selected before running any OCR engine.\n\n"
        "## Composition\n\n"
        f"{summary}\n\n"
        "The metric-based strata are sampling aids, not audited semantic labels. "
        "Manual merchant and quality labels remain blank in `test_manifest.csv` until reviewed.\n\n"
        "Do not replace images after observing OCR outputs. If the set must change, create a new "
        "version and document the reason.\n",
        encoding="utf-8",
    )
