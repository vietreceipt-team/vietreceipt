  ...
  host.innerHTML = `<header class="sticky top-0 z-50 h-14 border-b border-slate-200/90 bg-white/95 backdrop-blur">
    <div class="flex h-full w-full items-center gap-3 px-5">
      ...
      <div class="flex shrink-0 items-center gap-3">
        <span class="hidden text-right md:block"><span class="block text-xs font-bold leading-4 text-slate-800">Nguyễn An</span><span class="block text-[11px] leading-3 text-slate-400">Kiểm duyệt viên</span></span>
        <a href="/login/" class="grid size-9 place-items-center rounded-full text-xs font-bold leading-none transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-teal-600 focus-visible:ring-offset-2 ${pathname.startsWith("/login") ? "bg-teal-800 text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200"}" aria-label="Mở trang đăng nhập">NA</a>
      </div>
    </div>
  </header>`;
}
