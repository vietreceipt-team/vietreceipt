...
- **Status:** Ready for cross-owner review
...
- **Geometry:** Contract v1.3 đề xuất polygon bốn điểm, tọa độ chuẩn hóa trong `[0,1]`, gốc ở góc trên-trái. Quy ước này chỉ được coi là khóa chính thức sau khi OCR Owner sign-off.

### 2.2 Predicted value (`predicted_value`)

- **Định nghĩa:** Chuỗi giá trị mà KIE lựa chọn hoặc ghép từ một hay nhiều OCR block cho một field, trước bước chuẩn hóa.
- **Nguồn tạo:** Module KIE.
- **Quy tắc bất biến:** Không bị ghi đè bởi normalization hoặc chỉnh sửa của người dùng. Một lần chạy KIE mới phải được nhận diện bằng version/run riêng.
- **Yêu cầu truy vết:** Phải kèm `source_block_ids`, KIE version và field confidence nếu có.
...
- **Yêu cầu truy vết:** Phải xác định được normalization rule/version đã sử dụng.
...
- **Giá trị hiệu lực:** Backend tính `effective_value = corrected_value` khi `has_correction=true`; nếu không có correction thì dùng `normalized_value`. `effective_value` tuyệt đối không fallback sang `predicted_value`. Giá trị hiệu lực là giá trị dẫn xuất, không thay thế các lớp dữ liệu gốc.
...
Ưu tiên tên nhận diện người bán nằm gần đầu hóa đơn và có vai trò như header. Nếu đồng thời có tên thương hiệu và tên pháp nhân, ưu tiên tên thương hiệu hướng tới khách hàng; tên pháp nhân có thể được giữ trong candidate metadata để review. Không suy luận tên cửa hàng từ logo không có chữ hoặc từ kiến thức bên ngoài ảnh.
...
- Không tự suy ra năm bị thiếu hoặc tự chuyển năm hai chữ số nếu chưa có quy tắc đã được nhóm duyệt.
- Với biểu diễn số có thể hiểu theo nhiều thứ tự ngày/tháng, giữ predicted value nhưng để normalized value là `null` và yêu cầu review.
...
Không được tạo field value không có nguồn OCR mà không đánh dấu rõ nguồn suy ra. Mọi rule-based inference phải có version và có thể kiểm thử. Với field `PRESENT`, nhóm đề xuất `source_block_ids` chứa ít nhất một block; yêu cầu này chờ OCR/KIE sign-off cuối cùng.
...
| OCR polygon and coordinate convention | Proposed normalized four-point polygon | **NOTE — OCR Owner sign-off required** |
| OCR reading order | Proposed unique zero-based integer within OCR run | **NOTE — OCR Owner sign-off required** |
| Source IDs for `PRESENT` | Proposed minimum one OCR block | **NOTE — OCR and KIE Owners must confirm** |
| Review reasons when machine review is true | Proposed at least one reason | **NOTE — KIE and Backend must confirm schema enforcement** |
| Two-digit year | Do not infer until an approved deterministic rule exists | **NOTE — Team decision required before extractor implementation** |
| Brand versus legal entity | Prefer customer-facing brand; ambiguous cases require review | **NOTE — Confirm during annotation pilot** |
